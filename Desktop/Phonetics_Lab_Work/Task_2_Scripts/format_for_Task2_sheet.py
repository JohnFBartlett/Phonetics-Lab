...
def format_files(adjusted, breakfile, creakfile, reaperfile, measurements, out):
    ...
    with open(arel, 'r') as a:
        ...
        out.write('Filename' + '\t' + 'Segment' + '\t' + 'Tone' + '\t' + 'Next Seg Tone' + '\t' + 'Vowel?' + '\t' + 'Start Time(ms)' + '\t')
        ...
        if m:
            out.write('Creak' + '\t' + 'f0'  + '\t' + 'Pitchmark' + '\t' + str(next(m)))
        else:
            out.write('Creak' + '\t' + 'f0'  + '\t' + 'Pitchmark' + '\t' + "Measurements" + '\n')
        ...
        for line in a:
            ...
            if c:
                # blahbhjk = next(c)
                # print "next C line is " + blahbhjk
                try:
                    [cStart, cEnd, creakValue] = next(c).split('\t')
                    ccount += 1
            ...
            if r:
                try:
                    [rStart, rEnd, f0, pm] = next(r).split('\t')
                    rcount += 1
